...
class OpenLLMVTuberMain:
    ...
    def init_asr(self):
        ...
        return ASRFactory.get_asr_system(asr_model, **asr_config)
    ...
    def get_song_list(self) -> list[str]:
        ...
        if not os.path.exists(song_file_path):
            return []
        return [os.path.splitext(song)[0] for song in os.listdir(song_file_path)]
    ...
    def set_character_output_func(self, character_output_func) -> None:
        self.conversation_manager.set_character_output_func(character_output_func)
